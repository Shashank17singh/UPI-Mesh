...
bridge = BridgeIngestionService(
    crypto, idempotency, settlement, max_age_seconds=PACKET_MAX_AGE_SECONDS
)
...
def mesh_flush():
    ...
    def process(upload):
        with SessionLocal() as db:
            r = bridge.ingest(
                db, upload.packet, upload.bridge_node_id, 5 - upload.packet.ttl
            )
        return {
